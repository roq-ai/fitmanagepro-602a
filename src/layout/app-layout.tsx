...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiActivity } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'trainer', 'receptionist'];
  ...
  const { data, error, isLoading } = useSWR<GymInterface[]>(
    ...
    () => getGyms({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
